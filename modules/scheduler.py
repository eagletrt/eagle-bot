import logging
from modules.quiz import get_random_question, is_question_valid, get_question_answers, get_question_images, create_poll
...
async def send_scheduled_question(bot, group_id, thread_id, area_code):
    ...
    question = get_random_question(area_code)
    while question and not is_question_valid(question['id'], question['quiz_id']):
        question = get_random_question(area_code)

    answers = get_question_answers(question['id'], question['quiz_id'])
    images = get_question_images(question['id'], question['quiz_id'])

    qtext = f"Question {question['id']}-{question['quiz_id']} {question['type']} | {area_code}"
    ...
    if not options or not correct_indices:
        logging.warning(f"modules/scheduler - Question {question['id']}-{question['quiz_id']} has no answers or correct answer defined.")
        return
    ...
    logging.info(f"modules/scheduler - Scheduled question {question['id']}-{question['quiz_id']} | {area_code} sent to group {group_id} in thread {thread_id}.")
    ...
    create_poll(message.poll.id, question['id'], question['quiz_id'], correct_indices[0])
