import logging
from modules.quiz import get_question, get_question_answers, get_question_areas
...
async def answer(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    question_entity = get_question(question_id, quiz_id)
    if not question_entity:
        ...
        return
    answers = get_question_answers(question_id, quiz_id)
    areas = get_question_areas(question_id, quiz_id)
    ...
    logging.info(f"commands/answer - User @{username} requested correctly answers for question {question_id} in quiz {quiz_id} areas ({areas})")
